label beach_override:
    $locx = 20
    $locy = 15
    if not conversing_with == "Catalina":
        if minutes >= 360 and minutes < 1200:
            play music "audio/music/Guitar_Melancholy_loop.ogg" if_changed fadein 1.0 fadeout 1.0
        else:
            play music "audio/music/Guitar_Melancholy2_loop.ogg" if_changed fadein 1.0 fadeout 1.0

    if current_location == "Beach" and not is_outside:
        call check_weather 
        ...
        if (minutes >= 360 and minutes < 1200) or persistent.night_phases == "disable":
            if not dayphase == 1 or lost_consciousness:
                if current_weather == "rainy" or current_weather == "thunder":
                    scene bg beach day rain
                    with dissolve
                else:
                    play ambiance "audio/ambiance/Amb Beach 2.wav" fadein 1.0 fadeout 1.0
                    scene bg beach day
                    with dissolve
                $dayphase = 1
                $ is_in_public = True
    ...
    menu:
        ...
        "Relax" if currentenergy > 2.5:
            jump beach_relax
        "Buy snack" if currentenergy > 2.5 and minutes >= 480 and minutes < 1200:
            jump beach_snack
        "Go to the more secluded part of the beach":
            jump secluded_beach
        "Leave":
            jump city_map
